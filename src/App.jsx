...
export default function App() {
  ...
  const [alipayTotal, setAlipayTotal] = useState(null)
  ...
  const openAlipay = (acc, h, method = 'alipay', summary = null) => {
    ...
    setAlipayTotal(summary?.grandTotal || null)
    setActiveOrderId(summary?.orderId || null)
    setShowAlipay(true)
  }
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa] font-cn">
      <Navbar page={page} setPage={setPage} searchQ={searchQ} setSearchQ={(s) => { setSearchQ(s); setPage('home') }} onLogin={() => setShowLogin(true)} />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex md:hidden z-40">
        {[
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center py-2 gap-0.5 ${page === k ? 'text-brand' : 'text-gray-400'}`}
          >
