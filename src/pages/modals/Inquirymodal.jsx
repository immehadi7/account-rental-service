...
export default function InquiryModal({ acc, onClose, onPay }) {
  ...
  const handleSubmit = async () => {
    setError('')
    if (!user)    { setError('请先登录后再询单'); return }
    if (!contact) { setError('请输入联系方式');   return }
    ...
    >
      <div className="modal-enter bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400 text-lg"
          >
            ×
          </button>
        </div>
          ...
          {[1, 2, 3].map((s, i) => (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all ${
                ...
                step === s ? 'border-2 border-brand text-brand' :
        ...
        <div className="px-4 pb-4">
            ...
            <div>
              ...
              <div className="mb-2.5">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-brand transition-colors"
                />
              </div>
              ...
              <div className="mb-3">
                ...
                <textarea
                  ...
                  placeholder="特殊要求等..."
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-brand transition-colors resize-none h-14"
                />
              </div>

              {/* Error */}
              {error && (
                <div className="bg-red-50 border border-red-200 text-red-600 text-xs rounded-lg px-3 py-2 mb-2.5">
                  ⚠️ {error}
                </div>
              )}

              {/* Guarantee note */}
              <div className="bg-yellow-50 text-yellow-700 rounded-lg p-2.5 text-xs flex gap-1.5 mb-3">
                <span>⏳</span>
                <span>询单确认后方可付款，平台担保交付保障权益</span>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={onClose}
                  className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-xl text-sm font-semibold hover:bg-gray-50"
                >
                  取消
                </button>
                <button
          ...
          {step === 3 && (
            <div className="text-center">
              ...
              {order && (
                <div className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-1.5 mb-3 inline-block">
                  订单号：{order.orderNo}
                </div>
              )}
              ...
              <div className="flex gap-2">
                <button
                  onClick={onClose}
                  className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-xl text-sm font-semibold"
                >
                  关闭
                </button>
                <button
                  onClick={() => { onClose(); onPay(acc, hours) }}
                  className="flex-1 bg-[#1678ff] text-white py-2 rounded-xl text-sm font-bold"
                >
                  💙 立即付款
                </button>
              </div>
