...
function AccountCard({ acc, card, darkMode, onStatusToggle, onPriceUpdate }) {
  ...
  const savePrice = async () => {
    try {
      await API.put(`/accounts/${acc._id}`, { price: Number(newPrice) })
      onPriceUpdate(acc._id, Number(newPrice))
      setEditPrice(false)
    } catch (err) { console.error(err) }
  }

  const quickAdjust = async (delta) => {
    const updated = Math.max(1, acc.price + delta)
    try {
      await API.put(`/accounts/${acc._id}`, { price: updated })
      onPriceUpdate(acc._id, updated)
    } catch (err) { console.error(err) }
  }
...
export default function SellerDashboard({ setPage }) {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const pending = (ordRes.data.orders || []).filter(o => o.status === 'pending_confirmation')
      if (pending.length > 0) {
        setNotifications([{
          id: 1,
  ...
  return (
    <div className={`min-h-screen ${dm} transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* ── TABS ── */}
        <div className={`flex gap-1 mb-6 p-1 rounded-2xl border shadow-sm overflow-x-auto ${
          darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'
        }`}>
          {TABS.map(({ k, l, full }) => (
            <button
